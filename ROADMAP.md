...
- [ ] 7.1 **End-to-End manual testing:** Record $\rightarrow$ Transcribe $\rightarrow$ Stream $\rightarrow$ Display.
    ...
    - [ ] **7.1.2** `/chat` endpoint manual happy path — **blocked:** backend returned `422 VALIDATION_ERROR` on every send. Root cause: MEMORY.md Fork 1 wire format (persona prompt stuffed into `role`) is incompatible with the backend's standard LLM role enum. Contract revised — see `TECHNICAL_SPEC.md §API Contracts #2` + MEMORY.md Fork 4. Unblocked by 7.1.3.
    - [ ] **7.1.3** Implement revised `/chat` wire contract (Fork 4): three-field message shape (`role` enum + `role_prompt` + `content`). **Pre-condition:** backend deployment of the matching contract (single shared env — staging and production flavors point at the same backend URL per Fork 4 addendum §8). **Scope:** DTO + `ChatMapper` + `AppError` extension to cover `VALIDATION_ERROR` as Tier-1. Domain `ChatMessage.personaPrompt` and Room `ChatMessageEntity.personaPrompt` already exist (added in 5.5.B); no Room schema bump required. TDD breakdown:
        - [x] **7.1.3.A** `ChatMessageDto` + `ChatMapper` — shipped. DTO gained `@SerialName("role_prompt") val rolePrompt: String? = null`; mapper emits `role="user"` + `role_prompt=<persona>` + `content` on user turns and `role="assistant"` + `content` (no `role_prompt`) on assistant turns; throws `IllegalStateException` on any user message with null/blank `personaPrompt` (Fork 4 addendum §3, option A — fail-fast). `NetworkModule` Json config gained `explicitNulls = false` so null `role_prompt` on assistants is omitted from the wire. Collateral `ChatRepositoryImplTest` fixture updated with `tutorPrompt` to satisfy the new invariant.
        - [x] **7.1.3.B** `AppError.ValidationError` variant — shipped. New `data class AppError.ValidationError(rawMessage: String)` with `code = "VALIDATION_ERROR"`; `AppError.fromCode(...)` routes `"VALIDATION_ERROR"` to it, preserving the backend-supplied message verbatim (Fork 4 addendum §6 — backend 422 strings are byte-level pinned). `AppError.toChatEvent()` routes `ValidationError` → `ChatEvent.Tier1(message)`, so a 422 surfaces as a non-blocking Snackbar, not a Tier-3 AlertDialog. 237 unit tests green.
        - [x] **7.1.3.D** `ErrorMapper` parses the backend `{error:{code,message}}` envelope on 4xx / 5xx responses — shipped. New `ApiErrorEnvelope` + `ApiErrorDetail` DTOs under `data/remote/dto/`; `ErrorMapper` now injects `Json`, `fromThrowable` is `suspend`, and `ResponseException` handling splits into `readEnvelope()` (attempts JSON parse of the response body) / `classifyByStatus()` (fallback). Parseable bodies override status-based mapping, so any 4xx/5xx with `code:"VALIDATION_ERROR"` now routes via `AppError.fromCode(...)` → `ValidationError` → Tier-1 Snackbar. `ChatRepositoryImpl.toResourceError()` became `suspend` accordingly. 242 unit tests green. *Note:* the Ktor Logging plugin (DEBUG builds) reads response bodies; OkHttp engine buffers them so re-read by `ErrorMapper` should work, but 7.1.3.C / Check 2 is the definitive proof.
        - [ ] **7.1.3.C** Manual E2E re-run of 7.1.2 — send a fresh message against the backend, verify 200 OK + SSE stream arrives end-to-end, then mark 7.1.2 and 7.1 as `[x]`. Secondary check: point the client at a `mockable.io` endpoint returning `422 {"error":{"code":"VALIDATION_ERROR","message":"..."}}` and verify a Tier-1 Snackbar (not a Tier-3 AlertDialog) surfaces with the verbatim backend message. Requires 7.1.3.D landed first. *Note: Check 1 (happy path) was confirmed working 2026-04-18; Check 2 (Tier classification) pending 7.1.3.D.*
        - [ ] **7.1.3.E** *(queued as a later polish pass, not part of 7.1.3 core)* Tier-3 AlertDialog UX redesign — current implementation in `ChatScreen.kt` is functionally correct but needs Material 3 Expressive polish (icon, accent colour, hierarchy of title/message/action). Scope out during Phase 7.3 accessibility/polish work.
        - **Note on Room migrations:** if any *future* sub-task in 7.1.x needs a schema change, destructive migration is the preferred path (pre-launch, still in dev — no user data to preserve).
- [ ] **7.1.4** **Clean up orphaned assistant rows from failed streams** *(follow-up from Phase 7.1.2 Finding 3)*: a zero-length PENDING assistant row was observed persisting in Room after an earlier failed `/chat` call and re-poisoned every subsequent request. Per Fork 4 addendum §2 the backend now **accepts** empty-`content` assistant turns and silently drops them before forwarding to OpenAI — so this is **no longer a wire-level blocker**, just a client-hygiene task. Handle independently of 7.1.3; scope likely: delete or mark-FAILED the in-flight assistant row when `streamChat` errors before the first token, and/or filter empty-content assistant rows from the history replayed to `streamChat`.
