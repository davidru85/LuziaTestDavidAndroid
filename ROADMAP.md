...
#### 10.1 Code hygiene
- [ ] **10.1.A Deprecated API migrations** — Migrate `Icons.Filled.HelpOutline` → `Icons.AutoMirrored.Filled.HelpOutline` at [`ChatScreen.kt:204`](app/src/main/java/com/ruizurraca/luziatestdavid/presentation/screen/ChatScreen.kt#L204) and `hiltViewModel` → `androidx.hilt.lifecycle.viewmodel.compose.hiltViewModel` at [`ChatScreen.kt:43`](app/src/main/java/com/ruizurraca/luziatestdavid/presentation/screen/ChatScreen.kt#L43). Both flagged during Phase 8 build logs. Pure import swap. Effort: ~10 min.
- [ ] **10.1.B KT-73255 annotation-target opt-in** — Future-proof the `@Inject` constructor params in [`MediaRecorderAudioRecorder.kt:19-20`](app/src/main/java/com/ruizurraca/luziatestdavid/data/local/audio/MediaRecorderAudioRecorder.kt#L19-L20) and [`ChatRepositoryImpl.kt:34`](app/src/main/java/com/ruizurraca/luziatestdavid/data/repository/ChatRepositoryImpl.kt#L34) for Kotlin's upcoming default change (param-only → param+property). Either add `@param:` explicitly on each site, or opt in to `-Xannotation-default-target=param-property` globally. Effort: ~15 min + decision.

#### 10.2 Testing & coverage tooling
- [ ] **10.2.A Wire Jacoco coverage plugin** — Convert 9.1's audit-based coverage claim into a quantitative line/branch report. New catalog entry + `jacoco` plugin on the app module + `jacocoTestReport` task depending on `testStagingDebugUnitTest` + HTML artifact under `app/build/reports/jacoco/`. Optional: add a coverage gate to `.github/workflows/android-ci.yml`. Effort: ~30–60 min.
