...
- [ ] 7.3 **Accessibility, Polish & UX** — larger scope split into three sub-tasks per the `phase7_polish_deferred` memo + user-surfaced UX items. Absorbs:
    ...
    - **7.3.3 UI/UX Improvements** — six sub-tasks per user-surfaced suggestions + design decisions locked in `MEMORY.md §Phase 7.3.3 — UX Decisions Locked`. Ordering: A → B → C → D → E → F (quick wins first, i18n last so it translates the final stabilised copy).
        ...
        - [x] **7.3.3.C** Tier-3 AlertDialog — M3 Expressive redesign + Option-B copy. Shipped. `ChatEvent.Tier3.title: String` replaced with `Tier3.kind: Tier3Kind` (three variants: `ServiceUnavailable` / `InternalError` / `Unexpected`); optional `detailsMessage: String?` carries the backend verbatim string. New reusable `LuziaAlertDialog` composable under `presentation/component/` adds icon slot + severity tint + a collapsible **Details** section with expand/collapse toggle — also ready for 7.3.3.D to reuse. `ChatScreen` resolves kind → icon (`CloudOff` / `ErrorOutline` / `HelpOutline`) + `stringResource` title + friendly body via three private extension functions on `Tier3Kind`. 8 new strings pinned in `A11yStringMigrationTest`. Re-plumbs unblock 7.3.1's deferred title-i18n. 300 unit tests green.
        - [x] **7.3.3.D** Mic permission rationale + Clear conversation confirm dialogs — shipped. Both migrated to the `LuziaAlertDialog` wrapper introduced in 7.3.3.C. Mic rationale gains `Icons.Filled.Mic` with `primary` tint (informational). Clear-conversation gains `Icons.Outlined.DeleteSweep` with `error` tint (destructive action). No copy changes — existing strings read fine and are already pinned in `A11yStringMigrationTest`. Single `feat:` commit per the one-shot exception.
        - [x] **7.3.3.E** Role selector — persona icons + motion on selection. Shipped. Persistent leading icon per persona (`Icons.Outlined.School` / `Icons.Outlined.Science` / `Icons.Outlined.Palette`) — visible on both selected and unselected chips so each persona has a stable visual identity. Selected chip scales to 1.08× with a Material 3 `spring(DampingRatioMediumBouncy, StiffnessMedium)` bounce; the row uses `animateContentSize()` so scale changes settle smoothly. Existing `Role.RadioButton` / single-select semantics from 5.5.F unchanged. 2 new Compose tests assert the persona-icon `testTag` on each chip using `useUnmergedTree = true` (FilterChip merges leadingIcon semantics). 302 unit tests green.
