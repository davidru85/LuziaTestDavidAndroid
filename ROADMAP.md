...
- [ ] 7.2 **Edge Cases:** Test empty audio, network loss, and extremely long AI responses.
    ...
    - [x] **7.2.A** Preserve backend-supplied `message` strings verbatim in `AppError` variants — shipped (Option B, symmetric). The six formerly `data object` variants (`BadRequest` / `FileTooLarge` / `Timeout` / `Network` / `ServiceUnavailable` / `Internal`) are now `data class Variant(val rawMessage: String = "<generic default>")`. `AppError.fromCode(code, message)` threads the backend-supplied `message` through on every known code: preserves it verbatim when non-blank, falls back to the variant's generic default when null/blank. `ChatEvent.toChatEvent()` `when` branches switched to `is` patterns; `ErrorMapper` singleton references replaced with no-arg constructors. Six test files swept (`AppErrorTest`, `AppErrorToChatEventTest`, `ChatViewModelTest`, `ResourceTest`, `ChatRepositoryImplTest`, `ErrorMapperTest`) — `assertSame` → `assertEquals` for data-class equality, plus 12 new tests covering the preserved-message path per code. 262 unit tests green.
