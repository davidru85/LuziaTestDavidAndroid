...
- [ ] 7.3 **Accessibility, Polish & UX** — larger scope split into three sub-tasks per the `phase7_polish_deferred` memo + user-surfaced UX items. Absorbs:
    ...
    - **7.3.3 UI/UX Improvements** — six sub-tasks per user-surfaced suggestions + design decisions locked in `MEMORY.md §Phase 7.3.3 — UX Decisions Locked`. Ordering: A → B → C → D → E → F (quick wins first, i18n last so it translates the final stabilised copy).
        - [ ] **7.3.3.A** Record → Stop icon swap in `MorphingActionButton` while recording (fixes the affordance mismatch — today only `contentDescription` toggles, the visible icon stays `Mic`). Visual-only; existing `MorphingActionButtonTest` guards the semantic surface.
        - [ ] **7.3.3.B** FAILED assistant bubble copy — *"I've gone blank. Mind retrying?"* for the retryable latest failure; *"Sorry, empty message"* for older non-retryable empties. Signal disambiguates via the existing `onRetry: (() -> Unit)?` param. New strings in `strings.xml` for 7.3.3.F to translate.
