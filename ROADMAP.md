...
- [ ] 7.1 **End-to-End manual testing:** Record $\rightarrow$ Transcribe $\rightarrow$ Stream $\rightarrow$ Display.
    ...
    - [ ] **7.1.3** Implement revised `/chat` wire contract (Fork 4): three-field message shape (`role` enum + `role_prompt` + `content`). **Pre-condition:** backend deployment of the matching contract (single shared env — staging and production flavors point at the same backend URL per Fork 4 addendum §8). **Scope:** DTO + `ChatMapper` + `AppError` extension to cover `VALIDATION_ERROR` as Tier-1. Domain `ChatMessage.personaPrompt` and Room `ChatMessageEntity.personaPrompt` already exist (added in 5.5.B); no Room schema bump required. TDD breakdown:
        - [ ] **7.1.3.A** `ChatMessageDto` + `ChatMapper` — **🔴 RED:** update `ChatMapperTest` and add DTO serialization tests asserting the three-field JSON shape (`role="user"` + `role_prompt=<persona>` + `content` for user turns; `role="assistant"` + `content` + *no* `role_prompt` for assistant turns). Add a test asserting the mapper rejects / filters any user message with null or blank `personaPrompt` (strict-422 guard per Fork 4 addendum §3). **🟢 GREEN:** add `rolePrompt: String?` to `ChatMessageDto` with `@SerialName("role_prompt")`; rewrite `ChatMapper.toWireRole()` to emit the three-field shape. **🔵 REFACTOR:** verify domain purity + full unit suite green.
        - [ ] **7.1.3.B** `AppError` + `ErrorMapper` extension for `VALIDATION_ERROR` — **🔴 RED:** add `AppErrorTest` cases asserting `AppError.fromCode("VALIDATION_ERROR", ...)` yields a Tier-1-classified variant (not `Unknown`/Tier-3). Update `AppErrorToChatEventTest` to assert Tier-1 routing. **🟢 GREEN:** add `AppError.ValidationError` variant (or extend `BadRequest` family); update `AppError.toChatEvent()` mapping; update `ErrorMapper` as needed. **🔵 REFACTOR:** full suite + purity green.
