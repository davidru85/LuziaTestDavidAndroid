...
- [ ] 7.1 **End-to-End manual testing:** Record $\rightarrow$ Transcribe $\rightarrow$ Stream $\rightarrow$ Display.
    ...
    - [ ] **7.1.3** Implement revised `/chat` wire contract (Fork 4): add `role_prompt` field; `role` becomes enum `"user" | "assistant" | "system"`. **Pre-condition:** backend deployment of the matching three-field contract. **Scope is DTO + mapper only** — domain `ChatMessage.personaPrompt` and Room `ChatMessageEntity.personaPrompt` already exist (added in 5.5.B); no Room schema bump required. TDD breakdown:
        - [ ] **7.1.3.A** `ChatMessageDto` + `ChatMapper` — **🔴 RED:** update `ChatMapperTest` and add DTO serialization tests asserting the three-field JSON shape (`role="user"` + `role_prompt=<persona>` + `content` for user turns; `role="assistant"` + `content` + *no* `role_prompt` for assistant turns). **🟢 GREEN:** add `rolePrompt: String?` to `ChatMessageDto` with `@SerialName("role_prompt")`; rewrite `ChatMapper.toWireRole()` logic to emit `"user"` + separate `role_prompt`, or `"assistant"` + null `role_prompt`. **🔵 REFACTOR:** verify domain purity + full unit suite green.
        - [ ] **7.1.3.B** Manual E2E re-run of 7.1.2 — send a fresh message against the backend (once the backend has deployed the matching contract), verify 200 OK + SSE stream arrives end-to-end, then mark 7.1.2 as `[x]` and 7.1 as `[x]`.
...
- [ ] **7.1.4** **Clean up orphaned assistant rows from failed streams** *(follow-up from Phase 7.1.2 Finding 3)*: a zero-length PENDING assistant row was observed persisting in Room after an earlier failed `/chat` call and re-poisoned every subsequent request. Handle independently of 7.1.3; scope to be defined later (likely: delete or mark-FAILED the in-flight assistant row when `streamChat` errors before the first token, and/or filter empty-content assistant rows from the history passed to `streamChat`).
