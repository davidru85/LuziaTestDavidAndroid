...
- [ ] 7.3 **Accessibility, Polish & UX** — larger scope split into three sub-tasks per the `phase7_polish_deferred` memo + user-surfaced UX items. Absorbs:
    ...
    - **7.3.1 Accessibility** —
        - [x] **7.3.1.A** `stringResource` migration for all hardcoded user-visible / a11y English strings — 20 entries now in `res/values/strings.xml` (11 `cd_*` contentDescriptions + 8 `dialog_*` / clear-confirm copy + `input_placeholder`). Six components (`AssistantMessageBubble`, `UserMessageBubble`, `MorphingActionButton`, `ChatInputBar`, `ChatTopAppBar`) plus `ChatScreen` mic-rationale and Tier-3 AlertDialogs migrated to `stringResource(...)`. New `A11yStringMigrationTest` (Robolectric) pins each string's expected English value as a regression guard. 282 unit tests green. **Explicitly deferred to 7.3.3:** Tier-3 AlertDialog `title` strings (`"Service error"` / `"Unexpected error"`) — require re-plumbing `ChatEvent.Tier3` (move title selection to the composable via a semantic kind enum instead of a pre-resolved `String`); fits naturally alongside the AlertDialog UX redesign.
        - [x] **7.3.1.B** `LiveRegion.Polite` on assistant bubble for `LOADING` + `STREAMING` states — shipped. `LoadingLines` combines `contentDescription = "Loading response"` with `liveRegion = LiveRegionMode.Polite` so TalkBack announces the shimmer appearing; `AssistantText` gains an `announceChanges: Boolean` parameter that conditionally applies the live region (true for `STREAMING`, false for `RECEIVED`). `FAILED` and `RECEIVED` deliberately skip the live region so TalkBack doesn't re-announce history on scroll. 4 new semantics tests in `AssistantMessageBubbleTest` guard each branch; 286 unit tests green.
        - [ ] **7.3.1.C** Manual TalkBack verification on device — announcements sensible across the mic flow, persona chip selection, Snackbar errors, streaming assistant reply, clear-conversation dialog. Spot-check touch targets (≥48dp) and contrast (WCAG AA).
